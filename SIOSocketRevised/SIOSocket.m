...
//    [self.javascriptContext evaluateScript: [NSString stringWithFormat: @"objc_socket.emit(%@);", [arguments componentsJoinedByString: @", "]]];
    ...
    NSMutableString *emitParam = [@""mutableCopy];
    [emitParam appendFormat:@"'%@'", event];
    if(arguments && arguments.count){
        [emitParam appendString:@", "];
        [emitParam appendString:[arguments componentsJoinedByString: @", "]];
    }
    
    [self.javascriptContext evaluateScript: [NSString stringWithFormat: @"objc_socket.emit(%@);", emitParam]];
}
